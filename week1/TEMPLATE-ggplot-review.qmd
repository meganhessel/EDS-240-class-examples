...
ggplot(penguins, 
       aes(x = bill_length_mm, 
           ...
           color = species)) + 
  geom_point() + 
  scale_color_manual(values = c('coral', 'purple', 'cyan4'))
```
...
ggplot(penguins, aes(x = bill_length_mm, y = bill_depth_mm, color = body_mass_g)) + 
  geom_point() + 
  scale_color_gradient(low = "#F7DD4A", high = "#132B30")
```
...
ggplot(penguins, aes(x = species)) +
  geom_bar() +
  theme_light() + 
  theme(axis.title = element_text(size = 15, color = 'purple')
          )
```
...
(p1 + (p2 / p3)) + 
  plot_annotation(title = "All the greatest penguin info")
```