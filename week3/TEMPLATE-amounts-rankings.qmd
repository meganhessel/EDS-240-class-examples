...
subregion_names <- tribble( # Construct own datatable (tibble)
...
sui_severity <- ca_region |>
  ...
  mutate(month = month.abb[month], # replace all number months with month abbrevations 
         month = factor(month, levels = c(month.abb[10:12], month.abb[1:9]))) %>% # order months Oct > Sep (water year) first levels are Oct - Dec (water year) and then the rest follow 
  mutate(sui_category = factor(sui_category, 
                               levels = c("Very low", "Low", "Moderate", "High", "Severe")))
```
...
order_2015 <- heatmap_data |> 
  ...
  mutate(order = row_number()) |> # New column based on position in arranged dataframe 
