...
#metajam::download_d1_data("https://cn.dataone.org/cn/v2/resolve/https%3A%2F%2Fpasta.lternet.edu%2Fpackage%2Fdata%2Feml%2Fknb-lter-hbr%2F208%2F14%2F024b6acc5cb2e03a14fff5558bbffc0c",
                 #path = here::here("week4")) # drop here 

# Rename the downloaded folder to 'data/' so that it's automatically ignored by your repo's .gitignore!  
...
stream_chem_data <- read_csv(here::here("week4", "data", "HubbardBrook_weekly_stream_chemistry_1963-2025.csv"))
```
...
# relationship between DOC and pH
```
...
p2 # W9 information seems interesting 
...
stream_chem_data |> 
  ...
  geom_point(alpha = 0.5) +
  scale_size(range = c(1,10)) + # scales area of bubbles - 10 size difference between smallest and largest bubble 
  labs(x = "DOC (mg/L)", size = "Al (mg/L)", color = "Site")
...
stream_chem_data |> 
  ...
  ggplot(aes(x = DOC, y = pH, color = site, size = Al_ICP)) + 
  geom_point(alpha = 0.5) +
  scale_radius(range = c(1, 10)) + # BAD b/c scale by bubble radius - over exaggerates point differential 
  labs(x = "DOC (mg/L)", size = "Al (mg/L)", color = "Site")
```
...
ggplot(stream_chem_data, aes(x = SO4, y = pH)) + 
  geom_point()
#overplotting - cant look at trends 
```
...
#..........................contour plot..........................
ggplot(stream_chem_data, aes(x = SO4, y = pH)) +
  geom_density_2d_filled() # in 1by1in, 0-2% of data points vs 24-26% of data points 
```
