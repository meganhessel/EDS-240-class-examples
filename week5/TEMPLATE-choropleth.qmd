...
precip_data <- read_csv(here::here("week5", "data", "data.csv"), 
                        skip = 3)
...
joined_precip_geom <- full_join(county_geo_wrangled, precip_wrangled) # (x = sf, y = non-sf)
...
base_map <- ggplot(joined_precip_geom) + 
  ...
  labs(title = "5 year precip comapared with 20th century avg", 
       subtitle = "Jan 2020-Dec 2024", 
...
#| echo: true
# preview palette ----
RColorBrewer::display.brewer.pal(n = 11, name = 'BrBG') # Pick 11 colors from the package 

# save HEX codes as vector ----
my_brew_pal11 <- RColorBrewer::brewer.pal(n = 11, name = 'BrBG') # Pick 11 colors from the package 
...
base_map + 
  scale_fill_gradientn(colors = my_brew_pal11, 
                       labels = scales::label_percent(scale = 1), 
                       breaks = scales::breaks_width(width = 10), 
                       values = scales::rescale(x = c(-40, 0, 40)) # right end, middle, and left end 
                       ) + 
  guides(fill = guide_colorbar(barwidth = 25, barheight = 0.75))
         
# PROBLEM: 0 is not the nutral  
```
...
# save HEX codes as vector ----
my_brew_pal10 <- RColorBrewer::brewer.pal(n = 10, name = 'BrBG')
my_brew_pal10
```
